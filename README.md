...
## 当前状态：Phase 3 — 情绪状态深化 + 角色卡库 + 存档/读档

在 Phase 2（接真实 LLM）基础上，把它变成一个能长期玩的小应用：

- **角色卡库**：内置「綾 / 凛 / 雪」三张卡，可编辑、可新建自定义卡、可删除自定义卡。
  每张卡有自己的人设、说话风格、默认表情、初始情绪。点顶栏 🎭 管理。切换角色会开新会话。
- **会话自动持久化**：每轮对话后自动存到 localStorage，刷新页面 / 关掉重开都能恢复整段对话与情绪状态。
- **存档 / 读档**：点顶栏 💾 把当前对话存成命名快照，随时读回（含对话记录 + 情绪状态），可存多份。
- **emotion_state 多轮漂移**：情绪随对话连续变化，并跟随会话一起持久化。
- 🆕 新会话：一键清空当前对话，情绪回到角色卡初始状态。

两种运行模式不变：未配 Key = 演示模式（播样例）；配 Key = 实时模式（对话）。
...
### 流式低延迟（方案 A · 增量 beat 解析）

默认走流式（`stream:true`），**beat 边到边演**，不等整段 JSON 生成完：

- `createBeatStreamParser` 增量扫描累积文本，在 `"beats"` 数组范围内做括号配平，
  **每当一个 beat 对象闭合就立刻吐出**，喂给引擎。
- 引擎 `playStream()` 维护动态队列：有 beat 就播，没 beat 但流没结束就等待。
  于是**首字延迟从「整段生成时间」降到「第一个 beat 生成时间」**——角色几乎是 LLM 刚开口就开始演。
- `emotion_state` 在 beats 之后，等流结束再用主解析器补全。
- 调试台会显示「首字延迟 ms」与「总耗时 ms」。
- **自动兼容非流式网关**：若网关不返回 `event-stream`，自动回退到整段解析，行为不变。
- beat 协议契约**完全不动**（这是方案 A 相对「改成 JSONL」的优势）。
...
```
ArtiEmo/
├── index.html        # 舞台 + 输入框 + 设置/角色/存档面板 + 调试台
├── src/
│   ├── engine.js     # ★ 演出引擎：beat 队列播放器（核心，与渲染解耦，Phase 1 起不动）
│   ├── llm.js        # ★ BYOK LLM 客户端：OpenAI 兼容请求 + 容错 JSON 解析 + 规则层补全
│   ├── store.js      # ★ 存储层：角色卡库 + 会话持久化 + 命名存档（全在 localStorage）
│   ├── app.js        # 应用层：renderer + 双模式 + 角色/存档/会话管理 + 对话循环
...
└── data/
    ...
    └── character.js  # 默认角色卡库（綾/凛/雪，可被用户卡覆盖/扩充）
```

`engine.js` 是核心，且**与 DOM 解耦**——它只决定「何时该做什么」，
具体怎么画交给注入的 `renderer`。未来换 UI 框架/换渲染目标都不动引擎。
`llm.js` / `store.js` 也各自解耦：前者只负责「拿到合法 beat 序列」，后者只管「存取数据」。

## 路线图

- [x] **Phase 1** 演出引擎原型（模拟数据，验证沉浸感）
- [x] **Phase 2** 接真实 LLM：beat 协议 prompt + BYOK OpenAI 兼容客户端 + 规则层补全 + emotion_state 漂移
- [x] **Phase 3** 情绪状态深化：会话持久化 + 角色卡库（增删改/切换）+ 命名存档/读档 + 新会话
